 ...
 * @LastEditTime: 2021-12-18 20:51:44
...
- <a id="user_info">用户信息表 <font color="ffff00"> (user_info)</font></a>
  ...
  - 建表语句
    ...
    ```
  - 注：
    - 创建此表前要创建 [图像信息表](#img_info)、[角色表](#role_style)
...
- <a id="relation_obj_img">图片关系对应表 <font color="ffff00"> (relation_obj_img)</font></a>
  - 表名：relation_obj_img
  - 描述：店铺信息表
  - |字段名称|类型|中文名称|是否可空|默认值|注释| 
    |:----|:----|:----|:----|:----|:----|
    ...
    |fk_img_id|bigint(20)|图片id|N||外键，图片id，图片信息表主键|
    |spare1|varchar(10)|备用字段1|Y|||
    |spare2|varchar(50)|备用字段2|Y|||
    |spare3|varchar(100)|备用字段3|Y|||
...
- <a id="img_info">图片信息表 <font color="ffff00"> (img_info)</font></a>

  - 表名：img_info
  - 图片信息表
  - |字段名称|类型|中文名称|是否可空|默认值|注释| 
    |:----|:----|:----|:----|:----|:----|
    |img_id|bigint(20)|图片主键|N||自增id|
    |img_url|varchar(256)|图片url|N|||
    |img_seq|tinyint(4)|顺序号|Y|||
    |is_main|tinyint(4)|是否主图|N|0|0-是，1-否|
    |create_time|datetime|创建时间|N|||
    |spare1|varchar(10)|备用字段1|Y|||
    |spare2|varchar(50)|备用字段2|Y|||
    |spare3|varchar(100)|备用字段3|Y|||
